...
import { adminApi, AdminOrderSummary } from "@/lib/api";
  ...
  useEffect(() => {
    const loadOrders = async () => {
      try {
        ...
        const data = await withAdminTimeout(adminApi.getOrders(), "Admin orders request timed out.");
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
        ...
        <p className="mt-4 text-slate-500 font-medium text-lg">Memuatkan semakan pesanan...</p>
      </div>
    );
  }
      ...
      <header className="bg-white border-b border-slate-200 pt-16 pb-10 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-slate-500 leading-relaxed font-light text-lg">
            Paparan read-only untuk sejarah pesanan di peringkat Admin.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-10">
        {error ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center text-slate-500">
            Tiada pesanan untuk dipaparkan.
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {orders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-700">{order.user_name}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{order.user_email}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{order.status}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">RM {Number(order.total_amount).toFixed(2)}</td>
